...
const SPLIT_BILL_ABI = [
  ...
  "function getBill(uint256 _billId) view returns(string,uint256,address,bool)",
...
if (splitBillBtn) {
  splitBillBtn.addEventListener("click", async () => {
    try {
      ...
      alert("Creating bill on Arc Testnet...");

      await tx.wait();

      const billId = await contract.billCount();

      activityEl.innerHTML =
        "📋 Split Bill Created<br>" +
        "Bill ID: " + billId + "<br>" +
        "Name: " + billName + "<br>" +
        "Amount: " + amount + " USDC";
